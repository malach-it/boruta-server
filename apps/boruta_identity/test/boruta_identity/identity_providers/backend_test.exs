defmodule BorutaIdentity.IdentityProviders.BackendTest do
  ...
  describe "federated_login_url/2" do
    ...
    test "returns login url" do
      ...
      assert Backend.federated_login_url(backend, "name", "state") == "https://host.test/authorize?client_id=client_id&redirect_uri=http%3A%2F%2Flocalhost%3A4003%2Fbackends%2F#{backend.id}%2Fname%2Fcallback&response_type=code&scope=openid+email&state=state"
    end
  end
end
